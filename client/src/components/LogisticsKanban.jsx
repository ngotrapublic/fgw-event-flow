...
import { Clock, MapPin, AlertCircle, CheckCircle2, Package, Truck, ArrowRight, Sparkles } from 'lucide-react';
...
const COLUMNS = [
    { id: 'upcoming', title: 'Upcoming', subtitle: '5 Next Items', bg: 'bg-gradient-to-br from-violet-50/60 to-indigo-50/40', headerBg: 'bg-violet-500', icon: Package, limit: 5 },
    { id: 'setup', title: 'Setup', subtitle: 'Preparation', bg: 'bg-gradient-to-br from-amber-50/60 to-orange-50/40', headerBg: 'bg-amber-500', icon: Truck },
    { id: 'live', title: 'Live', subtitle: 'In Progress', bg: 'bg-gradient-to-br from-emerald-50/60 to-green-50/40', headerBg: 'bg-emerald-500', icon: CheckCircle2, isLive: true },
    { id: 'cleanup', title: 'Cleanup', subtitle: 'Wrapping up', bg: 'bg-gradient-to-br from-cyan-50/60 to-blue-50/40', headerBg: 'bg-cyan-500', icon: AlertCircle },
];
                    ...
                    {COLUMNS.map(col => (
                        <div key={col.id} className={cn(
                            ...
                            <div className="space-y-3 px-1 h-auto">
                                ...
                                {boardData[col.id]?.length === 0 && (
                                    <div className="py-10 border-2 border-dashed border-black/10 rounded-[24px] flex flex-col items-center justify-center gap-3 bg-white/30 backdrop-blur-sm">
                                        <div className={cn("p-3 rounded-xl border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]", col.headerBg)}>
                                            <col.icon size={20} className="text-white" />
                                        </div>
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No events</span>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex flex-col items-end gap-1">
                    {event.seriesEndDate && event.seriesEndDate !== event.eventDate ? (
                        <div className="flex items-center gap-1.5 px-2 py-0.5 bg-pink-50 text-pink-600 rounded-md text-[9px] font-black uppercase tracking-widest border border-pink-100">
                            <Sparkles size={8} fill="currentColor" />
                            {format(start, 'dd/MM')} - {format(new Date(event.seriesEndDate), 'dd/MM')}
                        </div>
                    ) : (
