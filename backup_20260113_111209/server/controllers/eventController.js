...
const auditService = require('../services/auditService'); // Phase 4 Step 4B
...
/**
 * Get events with optional date-range query and pagination
 * Supports: 
 *   - Date-range mode: startDate & endDate (cursor pagination)
 *   - All events mode: no date params (offset pagination)
 */
exports.getAllEvents = async (req, res, next) => {
    try {
        const { startDate, endDate, limit = 50, lastDocId, page = 1 } = req.query;
        const parsedLimit = Math.min(parseInt(limit) || 50, 100); // Max 100 for safety
        const parsedPage = parseInt(page) || 1;

        let query = eventsCollection.orderBy('eventDate', 'desc');

        // MODE 1: Date-range query (for performance optimization)
        if (startDate && endDate) {
            ...
            // Cursor-based pagination for date-range mode
            if (lastDocId) {
        ...
        // MODE 2: All events with offset pagination (for Dashboard list view)
        // First get total count for pagination info
        const countSnapshot = await eventsCollection.get();
        const totalCount = countSnapshot.size;
        const totalPages = Math.ceil(totalCount / parsedLimit);

        // Offset pagination: skip (page-1)*limit documents
...
exports.checkConflict = async (req, res, next) => {
    try {
        const conflict = await conflictService.checkConflict(req.body);
...
exports.createEvent = async (req, res, next) => {
    try {
        ...
        // Check for conflicts
        const conflict = await conflictService.checkConflict(eventData);
        if (conflict) {
            return res.status(409).json({
                error: 'Conflict detected',
                message: 'This event conflicts with an existing schedule.',
...
exports.updateEvent = async (req, res, next) => {
    try {
        ...
        const conflict = await conflictService.checkConflict(updatedEvent, id);
        if (conflict) {
            return res.status(409).json({
                error: 'Conflict detected',
