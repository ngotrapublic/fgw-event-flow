...
import {
    ...
    ChevronDown, Clock, Target, Award, Shield, Hammer, Mail, Phone, MapPin
} from 'lucide-react';
import {
...
const DepartmentGalaxy = () => {
    ...
    const fetchAnalytics = useCallback(async () => {
        try {
            ...
            console.log('[Analytics] Fetching:', startDate, '->', endDate, '| Period:', timePeriod);
            ...
            if (data.monthlyTrend > 10) {
                insights.push({ type: 'success', icon: TrendingUp, title: 'Tăng trưởng', text: `+${data.monthlyTrend}%`, subtext: 'so với tháng trước', color: 'emerald' });
            } else if (data.monthlyTrend < -10) {
                insights.push({ type: 'warning', icon: AlertTriangle, title: 'Cảnh báo', text: `${data.monthlyTrend}%`, subtext: 'so với tháng trước', color: 'amber' });
            } else {
                insights.push({ type: 'neutral', icon: Activity, title: 'Ổn định', text: `${data.monthlyTrend >= 0 ? '+' : ''}${data.monthlyTrend}%`, subtext: 'so với tháng trước', color: 'slate' });
            }
            if (data.topDepartment) {
                insights.push({ type: 'info', icon: Award, title: 'Top Performer', text: data.topDepartment.name, subtext: `${data.topDepartment.percent}% tổng sự kiện`, color: 'blue' });
            }
            // Top Location insight (replaces avg per week)
            if (data.topLocation) {
                insights.push({
                    type: 'location',
                    icon: MapPin,
                    title: 'Địa điểm Hot',
                    text: data.topLocation.name,
                    subtext: `${data.topLocation.count} sự kiện (${data.topLocation.percent}%)`,
                    color: 'violet'
                });
            } else {
                insights.push({ type: 'location', icon: MapPin, title: 'Địa điểm Hot', text: 'Chưa có', subtext: 'dữ liệu', color: 'violet' });
            }
    ...
    const { stats, totalEvents, eventsThisMonth, upcomingEvents, monthlyTrend, topDepartment, monthlyChartData, insights, heatmapData, pieData, securityStats, contactStats } = analyticsData;
    ...
    return (
        <div className="w-full animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                ...
                >
                    {(() => {
                        // Use contactStats from analytics API instead of events
                        ...
                        <div className="space-y-3 flex-1 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-100">
                            {contactStats.length === 0 ? (
                                ...
                                contactStats.map((contact, idx) => (
                                    <div key={contact.id || idx} className="group p-3 rounded-xl border-2 border-slate-100 hover:border-black hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all bg-white cursor-pointer overflow-hidden relative">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-[9px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100 uppercase tracking-tighter">{contact.department}</span>
                                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400">
                                                <Clock size={10} />
                                                {contact.time}
                                            </div>
                                        </div>
                                        <h4 className="font-black text-black text-sm truncate mb-2 uppercase tracking-tight group-hover:text-indigo-600 transition-colors">{contact.eventName}</h4>
