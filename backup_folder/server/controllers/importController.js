...
exports.getTemplate = async (req, res, next) => {
    try {
        ...
        sheet.addRow({
            ...
            facilities: 'Tivi: 1',
            note: 'Ví dụ về nhập ít thiết bị.'
        });
...
exports.importEvents = async (req, res, next) => {
    try {
        ...
        console.log(`[IMPORT] Headers found indices: Date=${idxDate}, Name=${idxName}, Dept=${idxDept}, Loc=${idxLoc}, Fac=${idxFac}`);
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                console.log(`[IMPORT] Row ${i + 2} Date Raw:`, dateStr);

                if (typeof dateStr === 'number') {
                    dateStr = new Date(Math.round((dateStr - 25569) * 864e5)).toISOString().split('T')[0];
                ...
                console.log(`[IMPORT] Row ${i + 2} Parsed Date:`, dateStr);
                ...
                const newEvent = {
                    ...
                    resources: [],
                    facilitiesSummary: '',
                    needsWelcomeNotification: true, // 🟢 New: for background notification
                    remindersSent: { oneDay: false, oneHour: false }
                };

                console.log(`[IMPORT] Saving row ${i + 2}:`, newEvent.eventName, newEvent.eventDate);
